...
def total(self):
    ...
    self.a.set(str(total_snacks_price*0.05,3)+"Rs")
    ...
    self.b.set(str(total_grocery_price*0.01,3)+"Rs")
    ...
    self.c.set(str(total_hygine_price*0.10,3)+"Rs")
    self.total_all_bill=(
        total_snacks_price+
        total_grocery_price+
...
def bill_area(self):
    self.intro(self)
    items = [
        {"name": "Nutella", "quantity": self.nutella.get(), "price": self.nu},
        {"name": "Noodles", "quantity": self.noodles.get(), "price": self.no},
        #...
    ]

    for item in items:
        if item["quantity"] > 0:
            self.txtarea.insert(END, f"{item['name']}\t\t {item['quantity']}\t{item['price']}\n")

    self.txtarea.insert(END, "------------------------------------\n")

    taxes = [
        {"name": "Snacks Tax", "amount": self.a.get()},
        {"name": "Grocery Tax", "amount": self.b.get()},
        {"name": "Beauty&Hygine Tax", "amount": self.c.get()},
    ]

    for tax in taxes:
        if tax["amount"]!= "0.0 Rs":
            self.txtarea.insert(END, f"Total {tax['name']} : {tax['amount']}\n")
